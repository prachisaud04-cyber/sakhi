...
export const JourneysScreen: React.FC<JourneysProps> = ({ go, goBack }) => {
  ...
  const [userJourneys, setUserJourneys] = useState<JourneyItem[]>(USER_RECENT_JOURNEYS)

  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        const stored = localStorage.getItem('sakhi_user_journeys')
        if (stored) {
          const parsed = JSON.parse(stored)
          if (Array.isArray(parsed) && parsed.length > 0) {
            setUserJourneys(parsed)
          }
        }
      } catch (e) {
        console.warn('Could not read stored user journeys', e)
      }
    }
  }, [])
  ...
  }, [userJourneys, filterCategory, sortBy])
