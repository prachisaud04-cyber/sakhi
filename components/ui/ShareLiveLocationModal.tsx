...
export const ShareLiveLocationModal: React.FC<ShareLiveLocationModalProps> = ({
  ...
  const handleStartSharingWhatsApp = () => {
    ...
    const liveLink = `${origin}/live/${newSession.sessionId}`

    // 3. Trigger Automated Gateway Dispatch in Background
    fetch('/api/dispatch', {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        location: location,
        recipients: selectedObj.map((c) => ({ name: c.name, phone: c.normalizedPhone })),
        batteryLevel: telemetry.batteryLevel,
        channels: ['sms', 'whatsapp'],
      }),
    }).catch((err) => console.warn('[Automated Gateway Notice]', err))

    // 4. Compose WhatsApp message with real battery status
    const durationLabel = durationMinutes > 0 ? `${durationMinutes} minutes` : 'Until I stop'
    const waText = `🚨 *SAKHI Live Location Sharing*\n\nI am sharing my real-time GPS location with you.\n\n📍 *View Live Telemetry Map:*\n${liveLink}\n\n🔋 *Battery:* ${telemetry.batteryStatusText}\n⏱ *Sharing Duration:* ${durationLabel}.\n\nPlease open the link to monitor my journey in real time.`

    setDeliveryStatus('Automated SMS & WhatsApp Gateway dispatched!')
    ...
    setTimeout(() => {
      ...
      setDeliveryStatus('Gateway dispatched & WhatsApp launched!')
      setTimeout(() => setDeliveryStatus(null), 4500)
    }, 400)
  }

  const shareableUrl = activeSession
    ? `${typeof window !== 'undefined' ? window.location.origin : ''}/live/${activeSession.sessionId}`
    : ''
